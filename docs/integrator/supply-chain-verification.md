...
- **Kyverno** — verify with `type: SigstoreBundle`; see Kyverno's
  ...
  guide.
- **Sigstore Policy Controller** — Sigstore-bundle support requires
  **v0.13.0+** and `signatureFormat: bundle`; see the
...
> Validated, cluster-tested copy-paste policies (Kyverno `SigstoreBundle` and
> Policy Controller `v0.13+`) are tracked in
> [#1537](https://github.com/NVIDIA/aicr/issues/1537). The earlier inline
> examples used the legacy Cosign format / a pre-bundle Policy Controller
> version and were removed rather than ship policy that silently fails to
> verify.
